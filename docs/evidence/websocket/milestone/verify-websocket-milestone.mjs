...
const root = resolve(import.meta.dirname, '../w');
const expected = JSON.parse(readFileSync(resolve(import.meta.dirname, 'websocket-inputs.json')));
...
const files = execFileSync('rg', ['--files', 'native', 'apps', 'packages', 'scripts', 'tests', 'docs/licenses/websocket'], { cwd: root, encoding: 'utf8' }).trim().split('\n').concat(['package.json', 'package-lock.json', 'tsconfig.json', 'vite.config.ts', 'playwright.config.ts']).sort();
...
const save = () => writeFileSync(resolve(import.meta.dirname, 'websocket-report.json'), JSON.stringify(report, null, 2) + '\n');
...
async function run(name, args) {
  ...
  const log = createWriteStream(resolve(import.meta.dirname, `websocket-final-${name}.log`));
...
try {
  ...
  await run('go-race', ['scripts/go.mjs', 'test', '-json', '-race', '-count=1', '-p=1', './transport', './webpeer', './app']);
