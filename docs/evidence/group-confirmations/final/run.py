...
out = root / '.cache/group-confirmations-final'
out.mkdir(parents=True, exist_ok=True)

def sources():
    names = subprocess.check_output(['rg', '--files', 'apps', 'packages', 'native', 'scripts', 'tests', 'package.json', 'package-lock.json', 'tsconfig.json', 'playwright.config.ts'], cwd=root, text=True).splitlines()
...
checks = [
    ...
    ('node', ['npm', 'test'], {}),
    ('go', ['node', 'scripts/go.mjs', 'test', '-json', '-race', '-count=1', '-p=1', './core', './transport', './app', './groups', './groupstore', './groupauthority', './groupaccess', './groupledger', './profilelock', './profilestate', './profilebinding', './profiledb', './sqlitedriver'], {}),
    ('native-build', ['npm', 'run', 'native:build'], {}),
    ('interop', ['npm', 'run', 'test:interop'], {}),
    ('cgo-boundary', ['node', 'scripts/go.mjs', 'test', '-json', '-race', '-count=1', '-p=1', '-tags', 'relayloom_sqlite_cgo,sqlite_omit_load_extension', '-run', 'TestGroupCommand|TestScopedRawProofTail|TestScopedRestrictivePrefix|TestGroupAdmission|TestGroupHistoricalDelete|TestGroupQuarantineReservation|TestGroupCorruptQuarantine|TestGroupHistoryExpiry|TestGroupOutbox|TestGroupSend|TestGroupConfirmation|TestStopSnapshot', './app', './groupauthority', './groupledger'], {}),
