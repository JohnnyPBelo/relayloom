...
## Latest application rebuild

After the bounded-history/cache/application fixes, `transport_hardening` rebuilt the CLI (SHA-256 `a3dc0344e357d91d9a9a92da9ad65af0fee254e39ddb00678b133075283b03e8`) and repeated2/2 mixed scenarios. Root then reran the broader `node --import tsx --test --test-concurrency=1 tests/native/*.test.ts`:3/3 passed in14.994s, including the vector test. Updated reports in `docs/evidence/native` identify the TCP run at22:35:38Z and serial run at22:35:47Z on2026-09-11. Root also ran all10 browser cases against this Go CLI successfully; these UI tests include drag/drop, media with synthetic capture, notification stubs, privacy-lock ordering, collections and history. This supersedes the older binary-specific result above for the current source; it remains host evidence, separate from the final Android APK/emulator results.
