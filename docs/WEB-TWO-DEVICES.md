...
Uma página já aberta pode continuar a executar uma versão antiga: a cache offline aguarda que os clientes antigos fechem, e **recarregar pode não activar a actualização**. Termine operações em curso, feche todos os separadores RelayLoom desse browser e reabra **https://johnnypbelo.github.io/relayloom/**. Desbloqueie o mesmo perfil, sem apagar os dados do site nem criar outra identidade. Fechar os separadores encerra ligações WebRTC e requer novos códigos. No IAB desta sessão, navegar pela página inicial fora do âmbito da cache activou a versão nova e preservou o perfil; isto não verifica os outros dispositivos físicos.
