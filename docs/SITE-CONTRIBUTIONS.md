...
O namespace privado `contribution-receipt` é separado da inbox e da fila do visitante. Limites: 64 KiB por preparação, 4 MiB agregados, com intenções dentro das 256 entradas/1 MiB da inbox e sujeito à quota real do perfil. O browser limpa em lotes para respeitar as 192 chaves por transacção existentes. Os métodos de assinatura/selagem não são RPCs. **Este incremento ainda não entrega o recibo pela rede nem encerra a fila do visitante por confirmação.** [Provas da persistência](evidence/site-contributions/receipt-persistence).
