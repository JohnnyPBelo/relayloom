...
Abra `http://127.0.0.1:4174/browser/index.html`. O servidor só distribui código; mensagens, chaves e armazenamento pertencem ao navegador. Não foi publicado um endereço de produção. [Funções, testes e limites](docs/WEB-APPLICATION.md): identidade, mensagens/outbox, social e sites têm percursos reais; grupos dinâmicos e a restante paridade continuam obrigatórios e pendentes. A instalação PWA é opcional. A app instalada já pode criar/revogar convites na UI para ligar a web no mesmo dispositivo; a rota web → WebSocket → Reticulum TCP/série PTY passou com partição e autora offline. A autora também pode ter apenas outro navegador como par WebRTC: esse intermediário encaminha para a app instalada sem conseguir ler a mensagem privada. [Evidência da rota completa](docs/evidence/rtc-reticulum).
