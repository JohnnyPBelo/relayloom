...
| FR-038 | Responsive drag/drop personal site builder | partial | `tests/browser/site-studio.spec.ts`, `tests/e2e/site-studio.spec.ts`, `tests/native/site-studio.test.ts` — Estúdio multipágina/13 blocos/media validado; gate final e controlos de corrupção/autoria/seeder passaram. Evidência: docs/evidence/site-studio; publicação HTTPS no passo seguinte. Toque físico/revisão independente pendentes. |
