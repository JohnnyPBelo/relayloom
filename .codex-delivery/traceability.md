...
| FR-007 | Group conversations and membership authorization | partial | `docs/GROUP-OUTBOX.md` — Gestão, admissão e autoridade de outbox por épocas verificadas em Node/Go; paragens e128+128 intenções, recuperação entre processos e UI de estado. Publicação/envio dinâmico, carriers, composição e revisão independente pendentes. |
...
| FR-012 | Durable offline outbox | partial | `docs/evidence/group-outbox/final` — Outbox Node/Go e reserva separada de pin; autoridade de grupo no mesmo commit, cancelamento local e seeding do autor controlados.256 intenções reais e saídas81/82 testadas; criação/envio dinâmico ainda instalado por fixture. Binários móveis novos pendentes. |
