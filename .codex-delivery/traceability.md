...
| FR-007 | Group conversations and membership authorization | partial | `docs/GROUP-SEND.md` — Gestão, admissão, autoridade e criação/envio/reply por APIs Node/Go; audiências de snapshots sem contactos globais. Testes dirigidos de TCP/falhas/mortes passaram. Confirmações automáticas, eventos, carriers, composição/gestão dinâmica na UI, plataformas e revisão independente pendentes; gate completo de host/UI/desktop passou. |
...
| FR-012 | Durable offline outbox | partial | `docs/GROUP-SEND.md` — Outbox por épocas com admissão/intenção no mesmo commit, reserva física antes de ready/rede, UUID e bytes originais após falha. Oito mortes reais entre motores e Unicode/compatibilidade antiga passaram. Confirmações de grupo, binários móveis e integração restante ainda pendentes. |
| FR-013 | Delivery and expiry states | partial | `tests/e2e/group-outbox.spec.ts` — Received/read históricos preservados; superseded e pausa por prova na API/UI real,17 percursos por núcleo e26 relatórios Axe sem violações. Composição dinâmica e gates móveis actuais pendentes. |
