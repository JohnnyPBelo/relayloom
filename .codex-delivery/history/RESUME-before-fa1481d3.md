...
## Active correction checkpoint — newer than the integrated milestones below

HEAD/origin are `3d6641a01cf1c0e6e2f79052f0e7ddb5d2314f4b`; all five milestone commits were pushed normally. CI run `34663513261` is terminal **failure**: Node passed Windows/macOS/Linux, Go unit/race passed, then mixed interoperability case5 (Go sender) failed at outbox-network.test.ts708 because status had crossed expiry while automatic pin still existed. Desktop/iOS jobs were skipped. This is not a408. Full failed log and job JSON are in `.cache/ci-34663513261-{failed.txt,result.json}`.

Root reproduced the expiry/pin invariant deterministically in both Node and Go by delaying an actual preparing→ready journal write across the deadline. Baseline focused tests failed; corrected state/send/retry controls passed Node3 and Go3 subcases with race. Source now carries one reconciliation observation time into state projection and reconciles mutation results coherently. See `docs/OUTBOX-SNAPSHOT-EXPIRY.md`. Sources are **uncommitted**; never reset/clean them. API/ACL/TTL/mixed controls were not weakened.

The sequential expiry validation session27363 completed and exited0: build,93 Node tests (82 application/existing +11 local group-certificate cases), Go11 core/11 transport/37 application top-level tests with race, rebuilt CLI, all5 mixed/vector cases,14 Node UI and14 Go UI. Sources remained frozen. Exact commands/outputs are in `.cache/expiry-consistency` and `docs/evidence/outbox/snapshot-expiry`. Current engine-source hash19f8ea…62cd; CLI d82008…a0b7. Mixed results were sourced from `.cache/native-mixed` and matched to that command's time window and binary hash; older published mixed reports remain unchanged. Do not rerun this completed gate without a new relevant change.

The native720×600 screenshot exposed horizontal onboarding overflow. The new `tests/e2e/onboarding-layout.spec.ts` reproduced772px document width at720px. Grid min-content constraints/decorative width and intermediate heading size were corrected. The next check found dark recovery-summary contrast2.08; its dark text now uses the accessible accent. A subsequent build/targeted Node test completed successfully at01:34:28UTC: eight widths, keyboard and light/dark Axe passed. Its exec handle69876 was unavailable after turn interruption, but the terminal Playwright `.cache/onboarding-layout-accessible/.last-run.json`, final result JSON and both screenshots were inspected; no blind restart was made. Current assets: `index-CFZzmFWN.js`/`index-DwCp7KCv.css`. Desktop smoke now checks actual document/viewport widths.

A new sequential final-layout runner records full15-case Node/Go UI and desktop build/smoke/package/packaged-smoke commands under `.cache/final-layout`. Revalidate its actual live exec handle before any restart and keep source inputs frozen until it finishes. Afterwards rebind Go/AAR and build/run exact-hash Android gates for the expiry correction plus final UI; APK27a71947 predates both corrections and must not inherit their evidence. No new agents or bridge/provider changes.

Separate uncommitted group certificate implementation exists in `packages/groups/src/certificates.ts` and `tests/group-certificates.test.ts`:11 real crypto cases and typecheck passed. No Go equivalent, registry, API, UI or dynamic-group claim yet. That work is paused for the CI correction. `GROUP-IMPLEMENTATION.md` records the next steps. Still no new/resumed agents.

## Earlier integrated milestone checkpoint
