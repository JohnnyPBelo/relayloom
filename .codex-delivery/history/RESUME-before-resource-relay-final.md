...
## Continuação após publicação — documentação commitada; contrato novo e falhasCI

HEADlocal7f67d1d (`Verify and document the published v3 resource library on HTTPS`), ainda nãoenviado nesta nota; origin7fdb76a. O commit guarda4ficheirosharness/guias/provaslive70ficheiros e rastreabilidade. O runtimewebpublicado continua7fdb76a/0fdbd1b9. WIPantigo preservado.

CI35618030583 terminouFAIL. TodososjobsPASSexceptoiOS eautonomous-browser. Browser101/102:116/123recursosdisponíveisnolimite15s. iOSbuild/startupPASS,addmedia60stimeout; testeUI funcionalnãoarrancou,semprova demensagem/foto.20artefactosiOSporhashverificados; curadoria docs/evidence/ci-7fdb76a aindaWIP. Nãoalterarpermissões/serviçosApple. Downloadslimitados3,1MBbrowser+419KBiOS;cache .cache/ci-7fdb76a.

Novo contrato WIP `packages/content/src/site-form.ts`, `packages/sites/src/contribution-protocol.ts`, `native/sites/{form,contribution,contribution_test}.go`, tests/site-contribution*.test.ts e .codex-delivery/SITE-CONTRIBUTIONS-IMPLEMENTATION.md.8testesPASS,46vectores9aceites/37recusados eassinaturaGo validadaNode/portátil. Nãointegradoemruntime/UI, não declararfuncionalidadepronta. Driver48224recolhido0;72099recolhido0. Preservar enquanto corrigeCI.

Correcção de desempenho emWIP: packages/browser/src/application.ts fazinspect readonlyfora dafila demutaçõescomguardsdegeração; resource-runtime.ts permiteinspectconcorrente,obtém/criaçãocontinuamserializadas. UIcontinualimitadaa4,cliente64;semalterarquotas/prazos. Teste tests/browser/site-resource-parallel.spec.ts reproduziu1emvezde4antes,passoudepois(4disponíveis,bloqueio/lockrecusamresultados). PrimeirofixtureerroubundleIddorecurso,preservadoantesdefixarreferência.operation.reference.bundleId. Gates6942FAILfixture,27319FAILserialização,34614PASSforamrecolhidos.

**Execuçãoactual:** handle7656,typecheck/build+13percursosChromiumresourceAPI/reference/budget/review/UI/concurrency, log .cache/resource-parallel-chromium.log, artefactos .cache/resource-parallel-chromium-artifacts. Revalidarhandle,nãorelancarsemestado. DepoisFirefox/WebKit/gatesafectados eCI. Fontepublicadaanteriornãoalterada. Semagentes/bridges/modelos/serviçosalterados. Nãoapagartrabalhodenovocontratoou64WIPoriginais;semcomplete/blocked.


## Continuação actual — contrato de contribuições e correcção de desempenho

O novo contrato TS/Go passou oito testes,46vectores(9aceites/37recusados) e umtesteGo/race de tipos. Conjunto com a fila UI:17testesNodePASS, typecheck/buildPASS. Ficheiros novos ainda WIP, sem runtime/API/UI de formulários. Hashes em .cache/site-contribution-contract-hashes.json. Preservar.

CI7fdb76a terminouFAIL:101/102browser(116/123refs em15s) e iOSseedphoto60stimeout apósstartupPASS. Os outrosjobsPASS, incluindoRNS e3pacotes.20artefactosiOS conferidos porhash; provas emdocs/evidence/ci-7fdb76a (WIP). Não inferir falhaAX ou mensagem entregue quando otesteUI nem começou. LimitaçãoApple continua.

Leitura readonly separada das filasdemutação em BrowserApplication/BrowserResourceRuntime, comguard degeração e segurançaantes/depois. Controlo real4leituras mostrou1antes,4depois; bloqueioe lockduranteI/Orecusamrespostas.13ChromiumPASS. Paraevitarautenticar123vezesomesmorecurso, ResourceInspector UIcoalesce apenasinspecçõesemcurso, por snapshot/página/referênciacompleta;semcachefinal. Cadaobtaincontinuaindividual. Abandono detodososconsumidoresremovejobdoregisto,masmantémslotatérespostareal;vistanovanãoreusaautorizaçõesantigas.9unitáriosPASS(incluídolimitedeconsumidores,isolamentodesnapshots/páginas/descritores,cloneporleitor,falhas,cancelamento).

O primeirotestecoalescingesperava4slotsocupadosmasagorahavia1pedido partilhado; corrigidaexpectativaparaprovarqueabrirnovavistagera2.ºpedidorealantesdelibertarorespondidoanterior. RelatórioFAILdeexpectativaantiga preservado .cache/resource-coalescing-first-report.json. Página128blocos/123refs agoraPASS7,8s; posted2,held2,peak2,bytesexactos,0erros(.cache/resource-coalescing-budget.log). Restantes7Chromiumderevisão/referênciapassaram. Semrelaxar15soulimites.

**Gateactivoagora:** handle6436,13Firefox(resourcesAPI/budget/parallel/reference/review/UI), log .cache/resource-coalescing-firefox.log. DepoisWebKit,regressãoapropriada eCI/pushdoajustededesempenho. Handles7656/50569/51867/26825foramrecolhidos(exit0/1/0/0respectivamente). 50569sófalhouexpectativadotesteantigo,comoacima. SourcepublicoHTTPcontinua7fdb76a/0fdbd1b9,semnenhumaalteração automática. HEADlocal7f67d1d(guia/provaslive)continuaporpush;ninguémalteroubridges/modelos/permissões/serviços,semagentesnovos.


## Candidata de desempenho e contribuições — gate actual97171

Novo gate **handle97171**, script `.cache/resource-performance-full.mjs`, relatório `.cache/resource-performance-full/report.json`, driver `.cache/resource-performance-full-driver.log`. Typecheck/contratos/Go sites-race/buildPASS; Chromium completo activo, depoisFirefox/WebKit, UI de recursos Node/Go e Linux build/run/package/run. São672fontes congeladas. Não alterar nem relançar sem revalidar handle/PID.

Desde os13Firefox e14WebKit dirigidosPASS, foi confirmado um interleaving novo: um caller interno podia alterar o objecto de pedido inspect→obtain enquanto BrowserApplication aguardava a projecção. O controlo devolveu payload antes da correcção (.cache/resource-read-intent-before.log). `parseSiteResourceRead` agora valida/copia os campos antes do primeiroawait na viareadonly e é reutilizado pelo leitor. Typecheck/build e2Chromium depoisPASS(contrato decontribuições realbrowser+controlo4leituras/bloqueio/lock/imutabilidade deintent). Este é um controlo da API interna, não alegação de exploração pela rede. Os testes/correcção ficam emWIP; HTTPSanterior nãoalterado.

Contrato decontribuições: TypeScript/Go com46vectores eassinaturacruzadaPASS; teste realbrowser+Go também passouWebKit eChromium. Ainda faltaFirefox e integração aplicação/armazenamento/transportes/caixa/owner-approval/UI/v4. A presença de módulos no source não entregaformulários naapp. Plano SITE-CONTRIBUTIONS-IMPLEMENTATION.md mantém oescopo inteiro.

Handle7656PASS13Chromium;50569FAILsomenteexpectativaantiga depedidos(antescoalescing),51867PASSbudget,26825PASS17contratos+Go tipo-race,6436PASS13Firefox,78942PASS14WebKit,27850FAILintentmutável,78220PASS2apóscorrecção. Todosrecolhidos. CI35618030583terminouFAIL(browser101/102lento;iOSseedfixturetimeout); demaisjobsPASS. ProvasWIP docs/evidence/ci-7fdb76a. Não alterarApplepermissões/serviços. HEADlocal7f67d1d ainda porpush,origin7fdb76a. Original64WIPpreservado porhash;45GiB livres. Semagentes/modelos/bridges/configuraçõesalterados. Próximo: terminar97171,rever/curar,commitar separadamente performance econtratoautónomo verificado, pushCIsemocultarfalhaApple, validar/actualizarpublicação quandoapropriado econtinuarruntime/UIdecontribuições/restocontrato.


Última observação do gate97171: handle e PID da fase Chromium confirmados vivos;672fontes inalteradas.33contratos eGo sites/race passaram; os percursos iniciais da matrizbrowser continuam aprovados. Semgatepúbliconovo nemnovopushnestaetapa. Esta continuação foi progressodeimplementação(contrato de contribuições TS/Go/browser, diagnósticoCI, correcção de desempenho/cancelamento/intent) seguida deesperaverificada. Não é bloqueio e não altera oobjectivointegral.


## Gate de desempenho 97171 — Chromium completo aprovado

Chromium terminou 104/104 testes, sem falhas, skips ou retries (10,7 minutos). Os 33 testes de contrato, typecheck, build e Go sites com race também passaram. Firefox está em execução; seguem WebKit, UI de recursos Node/Go e pacote Linux. O handle 97171 e o PID actual foram revalidados vivos. Relatório: `.cache/resource-performance-full/report.json`; driver: `.cache/resource-performance-full-driver.log`. As 672 fontes permanecem congeladas.

As provas iniciais do contrato de contribuições foram curadas em `docs/evidence/site-contributions/contract`, com o seu âmbito limitado explicitado. A integração de snapshot/formulário, journal/caixa, decisão do dono, transporte e UI ainda falta. A secção final de `SITE-CONTRIBUTIONS-IMPLEMENTATION.md` fixa essas fronteiras. Não chamar o contrato isolado de funcionalidade pronta.

A continuação produziu resultado de uma fase completa e revisão/documentação útil enquanto aguardava. Não é bloqueio nem conclusão do produto. Não reiniciar o gate nem publicar as fontes WIP antes de terminar a verificação apropriada.


## Gate97171 terminou com uma falha; retoma actual32327

O relatório original terminou FAIL: Chromium104PASS; Firefox103PASS/1FAIL. O handle97171 foi recolhido (retorno143; o relatório e o resultado Playwright registam a falha concluída da fase). O caso falhado era v1 RTC→WS→TCP via Go. O trace mostra `rtc.peer.offer()` a demorar61,7segundos antes da primeira publicação; não prova a causa da demora nem que a correcção de recursos foi exercitada. Sumário sem SDP/endereço/chaves em `.cache/resource-performance-network-timeout.json`.

Uma repetição dirigida, delimitada a três execuções do caso, passou3/3 em30,8segundos, sem modificar fontes, deadlines ou rede. Handle98610 recolhido0; relatório `.cache/resource-performance-network-repeat-report.json`. A causa histórica permanece por esclarecer, não reclassificar a falha como corrigida.

**Único gate activo:** handle32327, `.cache/resume-resource-performance.mjs`, driver `.cache/resource-performance-resumed-driver.log`, relatório `.cache/resource-performance-resumed/report.json`. Repete uma vez Firefox completo e depois executa WebKit, UI de recursos Node/Go e Linux build/run/package/run ainda pendentes. Reaproveita por hashes/proveniência as fases anteriores aprovadas; todas as672fontes foram comparadas e continuam congeladas. Se voltar a falhar, analisar a nova evidência antes de qualquer repetição, sem ciclo automático.

As fontes de desempenho e de contribuições continuam WIP, com HEADlocal7f67d1d/origin7fdb76a. A versão pública permanece0fdbd1b9 da fonte7fdb76a. Não declarar esta candidata publicada ou o produto concluído. A continuação recolheu evidência que altera o próximo passo e três controlos executados; é progresso seguido de espera verificada, não bloqueio.


## Retoma32327 — Firefox104PASS; WebKit activo

A matriz Firefox completa terminou104/104, sem falhas/skips/retries,13,0min. Chromium104PASS,33contratos,Go sites/race,typecheck/build permanecem aprovados pelos hashes/proveniência. O timeout anterior na ofertaRTC continua registado e não foi declarado corrigido. Os três controlos dirigidos e a retoma completa não estabeleceram a sua causa.

Handle32327 confirmado vivo e faseWebKit iniciada; depoisUI recursosNode/Go e pacoteLinux. Todasas672fontes continuam inalteradas. Relatório `.cache/resource-performance-resumed/report.json`, driver `.cache/resource-performance-resumed-driver.log`. Curador preparado `.cache/curate-resource-performance.mjs` exigePASS e hashesestáveis; ainda não executado. Não reiniciar gates nem publicar a candidata WIP antes do fim.

Esta continuação produziu um resultado completo novo e preparou a curadoria com falhas/proveniência preservadas; é progresso e espera verificada. O contrato de contribuições continua sem integração naapp. O HTML publicado continua7fdb76a/0fdbd1b9; HEADlocal7f67d1d semnovo push nesta etapa. Contrato global permanece activo, sem declaração de conclusão ou bloqueio.


## Revisão durante WebKit — relógio do contrato de contribuições

Foi executado um controlo criptográfico curto e independente da suite (não alterou fontes, relógioOS ou serviços): `.cache/contribution-clock-control.ts`/`.json`. Com60segundos de atraso no receptor, o envelope passa pela tolerância300000ms já existente emcoreNode/browser/Go, mas o novo verifyForForm recusa a criação futura estrita. Incompatibilidade confirmada no contrato ainda não integrado. A secção final de SITE-CONTRIBUTIONS-IMPLEMENTATION.md define o ajuste e controlos de fronteira paraDEPOISdo gate32327. Não esquecer essa correcção nem apresentar o contrato como pronto com base nos passes actuais.

Gate32327 continua activoWebKit, já ultrapassou os quatropercursos RTC/WS/TCP de sites; fontes672inalteradas. Nenhumagente/configuração/serviço alterado. A continuação produziu nova evidência relevante paraa próxima alteração, além daesperaverificada.


## Diagnóstico de revogação confirmado e relógio corrigido — 21 de Setembro

32327 terminou FAIL emWebKit(103PASS/1FAIL). O caso era managed relay revocation: oSOSchegou,o conteúdo interrompido não,mas Bfechoucomo invalid-frame. Framecapturado.C enviara um pedido enquanto Bservia; dadosoriginais preservados em `.cache/relay-revocation-original-webkit.json`. Não chamar o gate32327PASS nemexecutadasasfasesnativas/desktopque nãoarrancaram.

Novo teste determinista `tests/browser/relay-policy-race.spec.ts` retém a preparação de uma resposta automática, pausaorelayelibertaaresposta. Antes:request/servefalham(closedtrue,rejected1,erroRelaydesactivado);controlo de erroalheio comamesmamensagem érecusado(esperadoPASS). Primeirofixture tinha método inexistente profile.create; corrigidoparasignContent,preservandooerrodefixture. Logs `.cache/relay-policy-before*`,report `.cache/relay-policy-before-valid-report.json`,dados `.cache/relay-policy-before-data`.

Correcção `packages/browser/src/router.ts` define RelayRevokedError apenasparaocancelamentolocal; `mesh.ts` capturaessetipoapenasnaresposta automáticaainventário/pedido. Outroserroscontinuamarejeitaradmissão,semACKdesucesso. Nãoalteraframingoucriptografia,nemsuprimeerros porstring. Após:15WebKitPASS(3corridas×request/serve/erroalheio/corrupção/revogaçãooriginal),mesmalinkaberta eSOSpróprioentregue,semconteúdoautomaticodepoisdepausar. Provas `.cache/relay-policy-after.log`,`.cache/relay-policy-after-report.json`,`.cache/relay-policy-after-data`;97416recolhido0. Defeitoreproduzidocompatívelcomosframes,corrigido; nãoexplicaotimeoutRTCofferFirefoxseparado.

Patch de relógio aplicado emcontribution-protocol.ts/contribution.go e testes: tolerância300000msigualaoenvelope,expiraçãoeprazo30diasmantidos. Controlos exactamente300000aceite/300001recusado isoladosdaexpiraçãodosnapshot. `.cache/contribution-clock-control-fixed.json` confirmou que envelopeepropostaaceitamdesvio60s. OitoNodePASS,48vectores(11aceites/37recusados) TS/Go,Go sites-race13,7s,typecheckPASS. Handler39062recolhido0. Ocontratodecontribuiçõescontinuanãointegradoemapp/UI.

**Únicotesteactivoagora:** handle88268, `.cache/relay-clock-chromium.log`, specsrelay-policy-race,routingesite-contribution completos emChromium. DepoisvalidarFirefox/WebKit/negativosclock eactualizarosgatesafectados,semrepetiçãocíclica. Runtimepublico7fdb76a/0fdbd1b9permanece; HEADlocal7f67d1d eWIPnãoforampushed. Preserveold64WIPe todososnovoscontratos. Fontecongeladaapenasduranteexecuçõesemcurso; nãomarcarcomplete/blocked.


## Retoma efectiva — 21 de Setembro, validação de relay e relógio

O Firefox pendente terminou: `.cache/relay-clock-firefox.log`, 12 PASS em 35,0 s; handle 15505 já inexistente. Nesta retoma o WebKit equivalente terminou 12 PASS em 30,5 s (handle 86072 recolhido com código 0). Os relatórios exactos foram conservados em `.cache/relay-clock-{chromium,firefox,webkit}-report.json`; todos sem falhas, skips ou flaky. WIP preservado. Disco 19 GiB; reserva mínima 15 GiB.

Gate sequencial **activo: 29159**, `.cache/resource-relay-final.mjs`, relatório `.cache/resource-relay-final/report.json`. Typecheck, 33 contratos, Go sites/race e build PASS; WebKit completo com 107 casos em curso, depois UI recursos Node/Go e Linux build/run/package/run. Não iniciar outro gate pesado nem alterar fontes até terminar. Chromium 104 e Firefox 104 do gate anterior são mantidos com proveniência: só oito ficheiros mudaram depois (relay, relógio e respectivos testes); os 12 casos dirigidos por browser cobrem essas mudanças. Não reclassificar as matrizes anteriores como testes integralmente executados na fonte actual.

Novo commit local **995861d**: contrato de formulários/contribuições, oito ficheiros de código/testes e provas de 48 vectores. Não foi enviado ainda. O commit não mudou os bytes das fontes congeladas do gate. Não integra formulários no runtime/API/UI. As provas iniciais de 46 vectores foram preservadas separadamente das novas.

Nenhum agente foi criado ou retomado, conforme recuperação sequencial. Astra/Copilot Ultra e bridges/configurações intactos. Próximo: curar provas do gate com `.cache/curate-resource-relay-final.mjs`, rever e guardar correcções de desempenho/relay em commits coerentes; push normal autorizado; continuar documento v4, runtime/journal/inbox e UI real. O site público continua a usar fonte 7fdb76a / distribuição 0fdbd1b9. O contrato integral e a revisão independente continuam por concluir.
