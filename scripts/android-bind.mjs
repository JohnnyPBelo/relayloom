...
const output = resolve(cache, 'artifacts/relayloom-core.aar');
...
try {
  ...
  const report = { kind: 'ANDROID_AAR_BUILD', status: 'BUILT_NOT_DEVICE_TESTED', target: 'android/amd64', minApi: 24, go: 'go1.26.8', xMobile: pinnedMobile, aar: '.cache/android/artifacts/relayloom-core.aar', sha256, bytes: readFileSync(output).length, toolMetadata: versions, resourceBefore: before, command: ['gomobile', ...args.map(value => value.replace(root + '/', ''))] };
  writeFileSync(resolve(cache, 'aar-build-report.json'), JSON.stringify(report, null, 2) + '\n');
