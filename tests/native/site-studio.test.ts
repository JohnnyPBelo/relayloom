...
for (const authorRuntime of ["node", "go"] as Runtime[]) {
  ...
  test(
    `${authorRuntime} site → ${seedRuntime} restarted seeder → ${authorRuntime} new reader: encrypted multipage draft, authority, corruption and author-offline transfer`,
    { timeout: 90000 },
    async () => {
      const run = createRun("site-" + authorRuntime),
        clients: MixedClient[] = [],
        controls: string[] = [];
      let success = false;
      ...
      try {
        ...
        const alice = await a.call("setup", { name: "Site author", password }),
        ...
        assert.equal((await a.request("site-draft-load", {}, "")).status, 401);
        controls.push(
        ...
        writeReport("site-" + authorRuntime, {
