...
for (const backend of ["node", "native"] as const)
  test(
    `${backend} refuses forged, public and cross-reference receipt traffic with positive packets on the same live channel`,
    { timeout: 60000 },
    async () => {
      const visitor = await launch(undefined, 0, 0, backend),
        owner = createIdentity("Receipt authority"),
        other = createIdentity("Wrong receipt authority"),
        wire = new Router();
      try {
        ...
        const request = {
            ...
            formId: "form",
            values: {
              name: "RECEIPT_ADVERSARY_PRIVATE",
              count: 0,
              open: false,
            },
        ...
        const protocol = createContributionReceiptProtocol(
            nodeCertificateCrypto,
          ),
          intent = {
          ...
          receipt = protocol.create(owner, intent),
          ...
          seal = (
            ...
            ttl = intent.expires - intent.created,
          ) =>
            createBundleAt(
              actor,
        ...
        forged.receipt.signature = "A".repeat(88);
        const wrongReference = {
          type: "site-contribution-receipt",
          receipt: protocol.create(owner, {
            ...intent,
        ...
        const bad: [string, Bundle][] = [
          ...
          [
            "renewed-envelope",
            seal(content, owner, [card], intent.created + 1),
          ],
        ...
        for (const [name, bundle] of bad) {
          ...
          assert.equal(current.receipt, undefined, name);
          assert.equal(
            existsSync(
        ...
        assert.deepEqual(confirmed.operation.receipt, receipt);
