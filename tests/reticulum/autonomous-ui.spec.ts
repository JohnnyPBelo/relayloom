...
test("autonomous web UI reaches native reader through WS, reference RNS TCP and serial; partition, private reply and restarted seeder with browser author offline", async ({
  browser,
}) => {
  ...
  for (const page of [a, bp, c])
    page.on("pageerror", (e) => errors.push(e.message));
  const evidence = ".cache/reticulum-web";
  ...
  try {
    ...
    await network(a);
    await a.getByRole("tab", { name: "App instalada" }).click();
    await a.getByLabel("Convite da aplicação instalada").fill(invitation);
    await a.getByRole("button", { name: "Ligar ao nó", exact: true }).click();
    await expect(
      a.getByText("Ligação ao nó estabelecida.", { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      a
        .locator(".bubble")
        .getByText("Recebido, Alice. O caminho está aberto.", { exact: true }),
    ...
    const postText = "A biblioteca reabriu. Esta cópia fica com a comunidade.";
    ...
    expect((await f.c.call("view", { id: post.id })).author.id).toBe(alice.id);
