...
for (const documentVersion of [1, 2] as const)
  test(`v${documentVersion} native site studio restores full draft, renders edited posts and publishes verified pages to another process`, async ({
    browser,
  }) => {
    ...
    try {
      ...
      const expectedRuntime =
        process.env.RELAYLOOM_TEST_BACKEND === "native" ? "Go" : undefined;
      expect((await a.call("state")).nativeRuntime).toBe(expectedRuntime);
      expect((await b.call("state")).nativeRuntime).toBe(expectedRuntime);
      ...
      if (documentVersion === 2) {
        const cell = studio
          .getByRole("region", { name: "Editar dados da tabela", exact: true })
          .getByLabel("Linha 1, Litros", { exact: true });
        await cell.fill("");
        await cell.pressSequentially("2,5");
        await cell.press("Tab");
        await expect(cell).toHaveValue("2.5");
        value.site.pages[0].blocks.at(-1)!.data!.rows[0].values.water = 2.5;
      }
      ...
      expect(
        received.objects.find((o: any) => o.kind === "site").author.id,
      ).toBe(alice.id);
      ...
      if (documentVersion === 2) {
        ...
        await expect(
          table.getByRole("cell", { name: "2,5", exact: true }),
        ).toBeVisible();
        await dialog
        ...
        await expect(
          table.getByRole("cell", { name: "Parque", exact: true }),
        ).toBeVisible();
      }
      await dialog
        .getByRole("navigation", { name: "Páginas do site" })
