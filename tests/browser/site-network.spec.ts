...
const payload = (title: string, version: 1 | 2 = 1) => ({
  type: "site",
  blocks: [],
  theme: "sand",
  site: {
    ...
    pages: [
      {
        ...
        title: "Início",
        blocks: [
          { id: "body", type: "text", title: "Conteúdo", body: title },
          ...(version === 2
            ? [
                {
                  id: "data",
...
async function publish(
  ...
  return page.evaluate(
    ...
    { address, payload: payload(title, version), recipients },
  );
}
for (const version of [1, 2] as const)
  for (const bridge of ["node", "native"] as const)
    test(`version ${version} sites cross RTC, WebSocket and TCP through ${bridge}, heal a partition and survive with only a restarted reader`, async ({
      browser,
    }, info) => {
      ...
      try {
        ...
        expect(delivered.object.content.site).toEqual(
          payload("Edição um entre três meios", version).site,
        );
        ...
        await a.evaluate(
          async (card) =>
            (window as any).app.call("contact", { contact: card }),
          nativeReader,
        );
        ...
        expect(privateRead.object.content.site).toEqual(
          payload("Privado através de retransmissor sem chave", version).site,
        );
        ...
        const reply = await d.call("site-command", {
          ...
          payload: payload("Resposta nativa assinada", version),
          recipients: "public",
          ttlMs: 3600000,
        });
        await expect
        ...
        expect((await resolve(a, replyAddress)).object.content.site).toEqual(
          payload("Resposta nativa assinada", version).site,
        );
        ...
        await d.stop();
        expect(
          c.process.exitCode !== null || c.process.signalCode !== null,
        ).toBe(true);
        expect(
          d.process.exitCode !== null || d.process.signalCode !== null,
        ).toBe(true);
        ...
        expect(seeded.object.content.site).toEqual(
          payload("Edição dois preserva o endereço", version).site,
        );
        ...
        writeFileSync(
          `${directory}/${bridge}${version === 2 ? "-tables" : ""}.json`,
          JSON.stringify(
            {
              status: "PASS",
              documentVersion: version,
              typedTablePreserved: version === 2,
