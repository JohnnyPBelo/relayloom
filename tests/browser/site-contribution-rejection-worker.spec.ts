...
test("production worker recovers a private owner refusal after UI peering and offline reload while preserving a cancelled proposal", async ({
  browser,
}) => {
  ...
  try {
    ...
    const decision = {
      ...
      reason: "Não incorporar neste site.",
    };
    await expect(
      rpc(owner, "contribution-command", { ...decision, proof: original }),
    ).rejects.toThrow();
    await expect(
      rpc(owner, "contribution-command", decision),
    ).rejects.toThrow();
    await rpc(owner, "action", { action: "block", target: b.id, value: false });
    decision.revision = (
      await rpc(owner, "contribution-command", { action: "inbox" })
    ).management.revision;
    const declined = await rpc(owner, "contribution-command", decision);
    expect(declined.entry.phase).toBe("rejected");
    expect(declined.entry.verifiedAt).not.toBeNull();
    expect(declined.entry.proof).toBeNull();
    await owner.reload();
    await unlock(owner);
    expect(await rpc(owner, "contribution-command", decision)).toEqual(
      declined,
    );
    await rpc(owner, "action", { action: "block", target: b.id, value: false });
    expect(
    ...
    // Reconnect after the owner reload; only an authenticated decision closes the visitor queue.
    await connect();
    await expect
      .poll(
        async () =>
    ...
    expect(confirmed.rejection.body.certificateId).toBe(
      sent.operation.certificateId,
    );
    expect(confirmed.rejection.body.owner.id).toBe(a.id);
    expect(confirmed.rejection.body.contributorId).toBe(b.id);
    await expect(
    ...
    expect(confirmed.rejection.body.reason).toBe(decision.reason);
    await sender.evaluate(() => navigator.serviceWorker.ready.then(() => true));
    host.setUnavailable(true);
    await sender.reload();
    await unlock(sender);
