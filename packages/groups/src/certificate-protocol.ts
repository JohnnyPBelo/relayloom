...
/** Synchronous operations let authority transitions remain atomic in one transaction.
...
export function createGroupCertificateProtocol(crypto: GroupCertificateCrypto) {
  ...
  function base64(value: unknown, size: number): Uint8Array {
    requireThat(
    ...
    requireThat(
      bytes.length === size && encode64(bytes) === value,
  ...
  function memberCardHash(card: PublicIdentity): string {
  ...
  function checkCertificate<T>(
    ...
    requireThat(
      crypto.verify(signer, text, signature),
  ...
  function verifyGroupAnchor(value: unknown): GroupAnchor {
    const cert = record(value, ["body", "id", "signature"]);
  ...
  function createGroupAnchor(identity: Identity): GroupAnchor {
    return verifyGroupAnchor(
      signCertificate<AnchorBody>(
        {
          domain: "relayloom/group-anchor/1",
          creator: identity.public,
          nonce: encode64(crypto.random(32)),
          membershipAuthority: "creator-only",
        },
        identity,
        GROUP_LIMITS.certificateBytes,
  ...
  function verifyGroupInvitation(
    value: unknown,
    anchor: GroupAnchor,
    parent: GroupEpoch,
  ...
  function createGroupInvitation(
    ...
    const body: InvitationBody = {
      ...
      invitationNonce: encode64(crypto.random(32)),
    };
  ...
  function acceptGroupInvitation(
    ...
    const body: ConsentBody = {
      ...
      joinNonce: encode64(crypto.random(32)),
    };
  ...
  function createGroupLeave(
    ...
    const body: LeaveBody = {
      ...
      cardHash: memberCardHash(identity.public),
      leaveNonce: encode64(crypto.random(32)),
    };
  ...
  function verifyGroupSnapshot(
    ...
    if (head.body.number === 0)
      ...
      requireThat(
        snapshot.joins.every(
          (c) => c.body.parentEpochId === head.body.previous,
        ),
  ...
  function classifyGroupEpochPath(
    anchor: GroupAnchor,
    values: GroupEpoch[],
  ): "nonrestrictive" | "restrictive" {
  ...
  function verifyGroupTransition(
    anchor: GroupAnchor,
    parent: GroupEpoch,
  ...
  function verifyGroupSnapshotTransition(
    anchor: GroupAnchor,
    parent: GroupEpoch,
    next: GroupEpoch,
    newState: GroupSnapshot,
  ...
  function makeSnapshot(
    ...
    return snapshotBody({
      domain: "relayloom/group-state/1",
      groupId,
      salt: encode64(crypto.random(32)),
      title,
