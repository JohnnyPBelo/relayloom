...
func TestContributionReceiptWorker(t *testing.T) {
	path := os.Getenv("RELAYLOOM_CONTRIBUTION_RECEIPT_VECTORS")
	if path == "" {
		t.Skip("parent-owned receipt conformance fixture")
	}
	...
	for _, raw := range vectors {
		...
		case "create":
			...
			result, check = CreateContributionReceipt(identity, v["request"])
		case "verify":
			result, check = VerifyContributionReceipt(v["receipt"])
		case "match":
			result, check = MatchContributionReceipt(v["receipt"], v["proposal"])
		case "envelope":
			...
			if check == nil {
				result, check = MatchContributionReceiptEnvelope(bundle, v["plaintext"])
			}
		default:
