...
func (c *ContributionInbox) run(callback func(*PrivateRecords, map[string]any, *PrivateRecords, *PrivateRecords) error) error {
	return c.database.Update(func(tx *groupstore.Tx) error {
		return RunContributionInboxPrivate(tx, c.identity, func(values *PrivateRecords) error {
			return RunContributionReceiptPrivate(tx, c.identity, func(receipts *PrivateRecords) error {
				return RunContributionRejectionPrivate(tx, c.identity, func(rejections *PrivateRecords) error {
					...
					rejectionKeys, err := tx.Keys("contribution-rejection:")
					if err != nil {
						return err
					}
					var record map[string]any
					if !found {
						if len(keys) != 0 || len(receiptKeys) != 0 || len(rejectionKeys) != 0 {
					...
					expectedRejections := map[string]bool{}
					for _, raw := range record["entries"].([]any) {
						e := raw.(map[string]any)
						if op, ok := e["rejection"].(map[string]any); ok && op["stage"] != nil {
							expectedRejections[rejectionProofKey(docTextValue(e["id"]))] = true
						}
					}
					for key := range expectedRejections {
						_, exists, err := tx.Get(key)
						if err != nil {
							return err
						}
						if !exists {
							return privateIntegrity("preparação de recusa ausente")
						}
					}
					for _, key := range rejectionKeys {
						if !expectedRejections[inboxPartSuffix.ReplaceAllString(key, "")] {
							return privateIntegrity("preparação de recusa órfã")
						}
					}
					...
					if !creationEqual(next["revision"], record["revision"]) {
						for _, raw := range record["entries"].([]any) {
							...
							if op, ok := e["rejection"].(map[string]any); ok && op["stage"] != nil {
								var nextStage any
								if n != nil {
									if rejection, ok := n["rejection"].(map[string]any); ok {
										nextStage = rejection["stage"]
									}
								}
								if nextStage == nil {
									if _, err = c.readRejection(rejections, e); err != nil {
										return err
									}
									if err = rejections.Remove(rejectionProofKey(docTextValue(e["id"]))); err != nil {
										return err
									}
								}
							}
					...
					return callback(values, record, receipts, rejections)
				})
			})
		})
	})
...
func (c *ContributionInbox) State() (map[string]any, error) {
	...
	err := c.run(func(_ *PrivateRecords, r map[string]any, _ *PrivateRecords, _ *PrivateRecords) error {
		result = r
		return nil
	})
	if err != nil {
...
func (c *ContributionInbox) Admit(input core.Bundle, allow ContributionPolicy) (map[string]any, error) {
	...
	var result map[string]any
	err = c.run(func(values *PrivateRecords, record map[string]any, _ *PrivateRecords, _ *PrivateRecords) error {
		b := proposal["body"].(map[string]any)
		target := b["target"].(map[string]any)
		author := b["contributor"].(map[string]any)
...
func (c *ContributionInbox) SignReceipt(id string, allow ContributionPolicy) (map[string]any, error) {
	var result map[string]any
	err := c.run(func(values *PrivateRecords, record map[string]any, receipts *PrivateRecords, _ *PrivateRecords) error {
...
func (c *ContributionInbox) ReceiptBundle(id string, allow ContributionPolicy) (*core.Bundle, error) {
	...
	err := c.run(func(_ *PrivateRecords, record map[string]any, receipts *PrivateRecords, _ *PrivateRecords) error {
		e, err := c.receiptEntry(record, id, allow)
		if err != nil {
			return err
		}
...
func rejectionProofKey(id string) string { return "contribution-rejection:" + id + ":stage" }
func rejectionProofDescriptor(value any) (map[string]any, error) {
	b, err := core.Canonical(value)
	if err != nil {
		return nil, err
	}
...
func (c *ContributionInbox) readRejection(records *PrivateRecords, entry map[string]any) (map[string]any, error) {
	stage, err := c.readRejectionChecked(records, entry)
	if err != nil {
		return nil, privateIntegrity("preparação privada de recusa inválida")
	}
	return stage, nil
}
func (c *ContributionInbox) readRejectionChecked(records *PrivateRecords, entry map[string]any) (map[string]any, error) {
	op, ok := entry["rejection"].(map[string]any)
	if !ok || op["stage"] == nil {
		return nil, rejectionOperationError()
	}
	raw, found, err := records.Read(rejectionProofKey(docTextValue(entry["id"])))
	if err != nil {
		return nil, err
	}
	if !found {
	...
	stage, err := object(raw, "rejection", "envelope")
	if err != nil {
		return nil, err
	}
	desc, err := rejectionProofDescriptor(stage)
	if err != nil || !creationEqual(desc, op["stage"]) {
		return nil, rejectionOperationError()
	}
	rejection, err := CheckRejectionOperationCertificate(op, c.identity.Public.ID, entry, stage["rejection"])
	if err != nil {
		return nil, err
	}
	if op["phase"] == "signed" {
		...
		content, err := MatchContributionRejectionEnvelope(bundle, plain)
		if err != nil {
			return nil, err
		}
		transport := op["transport"].(map[string]any)
		bytes, _ := core.Canonical(bundle)
		if !creationEqual(content["rejection"], rejection) || transport["bundleId"] != bundle.Manifest.ID || transport["bundleHash"] != core.Hash(bytes) {
			return nil, rejectionOperationError()
		}
	}
	return stage, nil
}
func (c *ContributionInbox) rejectionEntry(record map[string]any, id string, allow ContributionPolicy) (map[string]any, error) {
	e := inboxEntry(record, id)
	if e == nil {
		return nil, errors.New("intenção de recusa indisponível")
	}
	op, ok := e["rejection"].(map[string]any)
	if !ok || op["phase"] == "expired" {
		return nil, errors.New("intenção de recusa indisponível")
	}
	target := e["target"].(map[string]any)
	...
	if !rejectionOperationLive(op, c.now()) {
		return nil, errors.New("recusa expirado")
	}
	return e, nil
}
func (c *ContributionInbox) SignRejection(id string, allow ContributionPolicy) (map[string]any, error) {
	var result map[string]any
	err := c.run(func(values *PrivateRecords, record map[string]any, _ *PrivateRecords, rejections *PrivateRecords) error {
		e, err := c.rejectionEntry(record, id, allow)
		if err != nil {
			return err
		}
		op := e["rejection"].(map[string]any)
		if op["phase"] != "prepared" {
			if _, err = c.readRejection(rejections, e); err != nil {
				return err
			}
		...
		_, found, err := rejections.Read(rejectionProofKey(id))
		if err != nil {
			return err
		}
		if found {
			return privateIntegrity("assinatura sem intenção de recusa")
		}
		identity := c.identity
		identity.Public, err = rejectionCard(op["owner"])
		if err != nil {
			return err
		}
		rejection, err := CreateContributionRejection(identity, op["request"])
		if err != nil {
			return err
		}
		stage := map[string]any{"rejection": rejection, "envelope": nil}
		desc, err := rejectionProofDescriptor(stage)
		if err != nil {
			return err
		}
		next, err := SignRejectionOperation(op, c.identity.Public.ID, e, rejection, desc, c.now())
		if err != nil {
			return err
		}
		record, result, err = UpdateInboxRejection(record, c.identity.Public.ID, id, next)
		if err != nil {
			return err
		}
		if err = rejections.Write(rejectionProofKey(id), stage); err != nil {
			return err
		}
		return values.Write(c.key+":record", record)
	})
...
func (c *ContributionInbox) SealRejection(id string, allow ContributionPolicy) (map[string]any, error) {
	var result map[string]any
	err := c.run(func(values *PrivateRecords, record map[string]any, _ *PrivateRecords, rejections *PrivateRecords) error {
		e, err := c.rejectionEntry(record, id, allow)
		if err != nil {
		...
		op := e["rejection"].(map[string]any)
		if op["phase"] == "prepared" {
			return errors.New("assinatura de recusa em falta")
		}
		stage, err := c.readRejection(rejections, e)
		if err != nil {
			return err
		}
		if op["phase"] == "queued" {
		...
		q := op["request"].(map[string]any)
		created, _ := contributionClock(q["decidedAt"])
		expires, _ := contributionClock(q["expires"])
		plain := map[string]any{"type": "site-contribution-rejection", "rejection": stage["rejection"]}
		bundle, err := core.CreateBundleAt(identity, "site-contribution-rejection", plain, []core.PublicIdentity{recipient}, false, expires-created, created)
		if err != nil {
			return err
		}
		if _, err = MatchContributionRejectionEnvelope(bundle, plain); err != nil {
			return err
		}
		stage["envelope"] = bundle
		desc, err := rejectionProofDescriptor(stage)
		...
		bytes, _ := core.Canonical(bundle)
		next, err := QueueRejectionOperation(op, c.identity.Public.ID, e, map[string]any{"id": bundle.Manifest.ID, "hash": core.Hash(bytes)}, desc, c.now())
		if err != nil {
			return err
		}
		record, result, err = UpdateInboxRejection(record, c.identity.Public.ID, id, next)
...
func (c *ContributionInbox) RejectionBundle(id string, allow ContributionPolicy) (*core.Bundle, error) {
	var result *core.Bundle
	err := c.run(func(_ *PrivateRecords, record map[string]any, _ *PrivateRecords, rejections *PrivateRecords) error {
		e, err := c.rejectionEntry(record, id, allow)
		if err != nil {
			return err
		}
		if e["rejection"].(map[string]any)["phase"] != "queued" {
			return nil
		}
		stage, err := c.readRejection(rejections, e)
		if err != nil {
...
func (c *ContributionInbox) CopyRejection(id string, actual core.Bundle, allow ContributionPolicy) (map[string]any, error) {
	bundle, err := inboxBundle(actual)
	if err != nil {
		return nil, err
	}
	...
	err = c.run(func(values *PrivateRecords, record map[string]any, _ *PrivateRecords, rejections *PrivateRecords) error {
		...
		if !creationEqual(stage["envelope"], bundle) {
			return privateIntegrity("cópia do recusa diferente")
		}
		bytes, _ := core.Canonical(bundle)
		next, err := CopyRejectionOperation(e["rejection"], c.identity.Public.ID, e, bundle.Manifest.ID, core.Hash(bytes), c.now())
		if err != nil {
			return err
		}
		updated, entry, err := UpdateInboxRejection(record, c.identity.Public.ID, id, next)
		if err != nil {
			return err
		}
		result = entry
...
func (c *ContributionInbox) Reject(id string, revision int64, reason string, allow ContributionPolicy) (map[string]any, error) {
	var result map[string]any
	err := c.run(func(values *PrivateRecords, record map[string]any, _ *PrivateRecords, _ *PrivateRecords) error {
		entry := inboxEntry(record, id)
		if entry == nil {
			return inboxError()
		}
		target := entry["target"].(map[string]any)
		if err := allow(docTextValue(target["snapshotId"]), docTextValue(entry["contributorId"])); err != nil {
			return err
		}
		if err := allow(docTextValue(target["snapshotId"]), c.identity.Public.ID); err != nil {
			return err
		...
		var proposal any
		if _, present := entry["rejection"]; !present {
			_, cert, err := c.readProof(values, entry)
			if err != nil {
				return err
			}
			proposal = cert
		}
		next, decided, err := RejectContributionInbox(record, c.identity.Public, id, revision, reason, proposal, c.now())
		if err != nil {
			return err
		}
		if !creationEqual(next["revision"], record["revision"]) {
		...
		result = map[string]any{"entry": decided, "revision": next["revision"]}
		return nil
	})
	return result, err
}
