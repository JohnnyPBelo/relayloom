...
func TestReceiptRuntimeBudgetWorker(t *testing.T) {
	path := os.Getenv("RELAYLOOM_RECEIPT_BUDGET_FORM")
	if path == "" {
		...
		func() {
			...
			count := func(state map[string]any, phase string, copied bool) int {
				...
				for _, raw := range state["entries"].([]any) {
					op := raw.(map[string]any)["receipt"].(map[string]any)
					if op["phase"] != phase {
						continue
					}
					if copied {
			...
			ids := []string{}
			for _, raw := range first["entries"].([]any) {
				op := raw.(map[string]any)["receipt"].(map[string]any)
			...
			for i, id := range ids {
				e := second["entries"].([]any)[i].(map[string]any)
				if e["receipt"].(map[string]any)["transport"].(map[string]any)["bundleId"] != id {
