...
func StartWithOptions(dataDir, assetsPath string, options Options) (*Service, error) {
	...
	service.server.Handler = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasPrefix(requestPath, "/api/") {
			...
			if !contains([]string{"ui-preferences", "setup", "unlock", "lock", "export", "contact", "connect", "web-peer", "web-peer-stop", "serial", "site-draft", "site-draft-load", "site-command", "resource-command", "contribution-command", "collection", "retrieve", "publish", "send", "outbox-retry", "group-command", "action", "settings", "view", "history", "attachment"}, operation) {
