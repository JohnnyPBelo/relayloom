...
O próximo incremento acrescenta decisões privadas sobre propostas enviadas às páginas. O protocolo, a persistência e a entrega de recusas estão verificados localmente; a aprovação/publicação com proveniência e a interface completa continuam por integrar. O [CI anterior](docs/evidence/ci-9a56c5f) falhou no limite de um auxiliar Go nos três hosts; a causa foi reproduzida e o [driver corrigido passou localmente](docs/evidence/ci-receipt-budget-cold), aguardando novo CI. O HTML público mantém a versão anterior.
...
A biblioteca de **recursos opcionais** já está na [versão web](https://johnnypbelo.github.io/relayloom/): em **A minha página → Recursos**, cria ficheiros ou tabelas, pesquisa, pré-visualiza e insere referências em páginas v3 assinadas. A obtenção é explícita; cada recurso conserva o seu autor e os leitores podem servir cópias. Foram conferidos os 20 ficheiros HTTPS e passaram seis percursos de recursos nos três browsers, mais mensagens/anexo e recuperação offline entre processos independentes. **Isso não equivale a dispositivos físicos testados nem conclui o produto.** [Provas da publicação](docs/evidence/site-optional-resources/v3-ui/live) · [Contrato e limites](docs/SITE-RESOURCES.md).

O código actual inclui um novo bloco **Tabela**: colunas tipadas, edição de linhas, CSV/JSON, pesquisa, ordenação e paginação, dentro de versões assinadas. A extensão passou 138 testes Node, Go/race, 11 de interoperabilidade, oito percursos UI Node/Go e 108 de browser. **Já está na versão web publicada**, na fonte `59c9bd1`/distribuição `f6758222`: 19 ficheiros HTTPS conferidos, 15 percursos de páginas nos três browsers e um percurso entre processos independentes passaram. [Provas da publicação](docs/evidence/site-data/live). [Guia e limites](docs/SITE-DATA.md) · [Provas, incluindo falhas corrigidas](docs/evidence/site-data). Contribuições multiutilizador continuam por implementar; os recursos opcionais v3 estão agora publicados, conforme as provas acima.

O novo estúdio já está disponível na [versão web](https://johnnypbelo.github.io/relayloom/). Em **A minha página**, permite criar um site com várias páginas, 15 tipos de bloco, colunas aninhadas, galerias, estilos, três modelos e Markdown seguro. Inclui pré-visualização móvel, desfazer/refazer e importação/exportação declarativa. O rascunho é cifrado; publicar guarda o projecto actual e assina o conteúdo, que os leitores podem distribuir sem adquirir autoria.
...
A próxima extensão são **formulários e contribuições assinadas**. Os motores Node/Go/browser já têm envio/inbox privados, origem recuperável, descarte e recibos assinados entregues entre pares. A confirmação fecha a fila sem conferir aprovação. A integração passou 531 testes Node, Go/race, 135 testes entre processos e 110 casos por navegador. **A recusa comunicada já funciona nos motores; faltam aprovação/incorporação, reconciliação/proveniência e o fluxo completo na interface. O HTML público ainda não oferece esse fluxo.** [Estado funcional](docs/SITE-CONTRIBUTIONS.md) · [Provas e limites](docs/evidence/site-contributions/receipt-delivery).

Esta adaptação inspira-se nos sites distribuídos do ZeroNet, com execução limitada a blocos seguros. Os limites actuais são 12 páginas, 128 blocos, três níveis e quatro imagens com até 2 MiB no total. Na interface publicada ainda faltam contribuições multiutilizador; os ficheiros opcionais, endereço permanente, histórico e recuperação já estão disponíveis. [Como criar e limites exactos](docs/SITE-STUDIO.md).
...
Foi reproduzida localmente uma corrida em que pausar o relay durante a preparação de uma resposta automática fechava a ligação. A correcção distingue essa mudança de consentimento de um frame inválido; os controlos de pedido, resposta e erro alheio passaram nos três browsers. A validação local passou 107 casos WebKit, dois percursos de recursos com Node/Go e a execução do pacote Linux. [Provas, controlos e proveniência](docs/evidence/site-resource-performance). Isso não estabelece a causa de todas as ocorrências históricas de ICE ou timeout de criação de oferta. O HTML público usa agora a fonte **7fdb76a**, na distribuição **0fdbd1b9**, com tabelas, recursos e o estúdio de versões. [Evidência actual da publicação](docs/evidence/site-optional-resources/v3-ui/live). A publicação anterior do estúdio está preservada em [evidência histórica](docs/evidence/site-editor-versions/live). [Âmbito e limites](docs/SITE-REVISIONS.md).
