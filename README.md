...
A próxima extensão são **formulários e contribuições assinadas**. O código liga formulários a tabelas autenticadas e guarda intenções, assinaturas e envelopes privados recuperáveis em Node/Go/browser. A integração local acrescenta fila e API de envio, com candidatos verificáveis no destinatário. **Faltam decisões duráveis, recibos, aprovação e o fluxo completo na interface; o HTML público não oferece esse fluxo.** O marco passou 505 testes Node, Go/race, 35 testes entre processos e 49 percursos por browser; uma correcção final teve revalidação de 16 casos afectados por engine, com proveniência separada. [Estado verificável](docs/STATUS.md) · [Provas e falhas corrigidas](docs/evidence/site-contributions/submission). [Comportamento e limites](docs/SITE-CONTRIBUTIONS.md).
