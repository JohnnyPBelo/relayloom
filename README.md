...
No checkpoint `bff00cc`, o CI34697388472 passou Node/Go e os três pacotes desktop. O gate iOS26.4.1 voltou a falhar na importação da fotografia de teste, antes do XCUITest. Compilação/instalação não demonstram execução da aplicação; ver [STATUS](docs/STATUS.md).

A gestão de membros por épocas tem bibliotecas Node/Go e uma API local autenticada, com convite, consentimento, saída/remoção/reentrada e persistência transaccional cifrada. Os testes usam processos e ficheiros reais, incluindo troca de núcleo no reinício. A interface ainda usa grupos fixos; envio dinâmico, sincronização automática das provas, admissão/outbox e UI estão pendentes. [GROUP-RUNTIME](docs/GROUP-RUNTIME.md) separa estas capacidades e os testes executados.
