...
- As conversas da interface têm membros fixos. A [gestão por épocas](docs/GROUP-RUNTIME.md) está implementada na API, incluindo envio pela rede quando as provas estão disponíveis; composição/gestão dinâmica e carriers de provas na interface continuam pendentes. Rotação/revogação também não está concluída. Máximo 64 destinatários, 4 MiB por objecto, quatro anexos por mensagem e 2 MB por anexo na interface.
