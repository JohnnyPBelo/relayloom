...
A próxima extensão são **formulários e contribuições assinadas**. Os motores Node/Go/browser já têm envio/inbox privados, origem recuperável, descarte e recibos assinados entregues entre pares. A confirmação fecha a fila sem conferir aprovação. A integração passou 531 testes Node, Go/race, 135 testes entre processos e 110 casos por navegador. **Faltam aprovação/recusa comunicada, reconciliação/proveniência e o fluxo completo na interface; o HTML público ainda não oferece esse fluxo.** [Estado funcional](docs/SITE-CONTRIBUTIONS.md) · [Provas e limites](docs/evidence/site-contributions/receipt-delivery).
