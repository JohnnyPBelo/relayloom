...
Nesta branch, a biblioteca de **recursos opcionais** já está ligada ao editor e ao leitor: ficheiros, tabelas importáveis, pesquisa, pré-visualização e inserção em páginas v3 assinadas. Node, Go e browser verificam referência, autor, bytes e leitores; a obtenção é explícita e o leitor pode servir a cópia sem adquirir autoria. Passaram306 execuções de browser,68 UI Node/Go, os gates de backend com hashes correspondentes, o pacote Linux e percursos com a referência Reticulum. Foram corrigidas falhas de contraste, foco, preview e capacidade de páginas extensas. **Esta extensão ainda não está no HTML público.** [Contrato e limites](docs/SITE-RESOURCES.md) · [Provas, falhas e âmbito](docs/evidence/site-optional-resources/v3-ui).

O código actual inclui um novo bloco **Tabela**: colunas tipadas, edição de linhas, CSV/JSON, pesquisa, ordenação e paginação, dentro de versões assinadas. A extensão passou 138 testes Node, Go/race, 11 de interoperabilidade, oito percursos UI Node/Go e 108 de browser. **Já está na versão web publicada**, na fonte `59c9bd1`/distribuição `f6758222`: 19 ficheiros HTTPS conferidos, 15 percursos de páginas nos três browsers e um percurso entre processos independentes passaram. [Provas da publicação](docs/evidence/site-data/live). [Guia e limites](docs/SITE-DATA.md) · [Provas, incluindo falhas corrigidas](docs/evidence/site-data). Contribuições multiutilizador continuam por implementar; os recursos opcionais verificados localmente aguardam publicação.

O novo estúdio já está disponível na [versão web](https://johnnypbelo.github.io/relayloom/). Em **A minha página**, permite criar um site com várias páginas, 14 tipos de bloco, colunas aninhadas, galerias, estilos, três modelos e Markdown seguro. Inclui pré-visualização móvel, desfazer/refazer e importação/exportação declarativa. O rascunho é cifrado; publicar guarda o projecto actual e assina o conteúdo, que os leitores podem distribuir sem adquirir autoria.
