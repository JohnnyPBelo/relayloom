...
A próxima extensão são **formulários e contribuições assinadas**. O código já liga formulários a tabelas autenticadas e guarda intenções, assinaturas e envelopes privados recuperáveis em Node/Go/browser. **Ainda faltam envio, caixa de propostas e aprovação na interface; o HTML público não oferece esse fluxo.** O marco local passou 500 testes Node, 18 testes entre processos e 21 percursos afectados em cada browser. [Estado funcional e limites](docs/SITE-CONTRIBUTIONS.md) · [Provas e falhas corrigidas](docs/evidence/site-contributions/envelopes).
