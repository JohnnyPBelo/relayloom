...
A gestão de membros por épocas tem bibliotecas Node/Go e uma API local autenticada, com convite, consentimento, saída/remoção/reentrada e persistência transaccional cifrada. Os testes usam processos e ficheiros reais, incluindo troca de núcleo no reinício. A recepção/admissão por épocas já usa transacções e reservas físicas verificadas em Node/Go. A interface ainda usa grupos fixos. A autoridade da outbox dinâmica está em integração local, com paragens transaccionais e guardas de retransmissão/partilha; [GROUP-OUTBOX](docs/GROUP-OUTBOX.md) separa código e evidência. Criação/envio dinâmico, sincronização automática das provas e a sua interface continuam pendentes. [GROUP-RUNTIME](docs/GROUP-RUNTIME.md) separa estas capacidades e os testes executados.
