...
O código actual inclui um novo bloco **Tabela**: colunas tipadas, edição de linhas, CSV/JSON, pesquisa, ordenação e paginação, dentro de versões assinadas. A extensão passou 138 testes Node, Go/race, 11 de interoperabilidade, oito percursos UI Node/Go e 108 de browser. **Já está na versão web publicada** (fonte 59c9bd1, distribuição f6758222). [Guia e limites](docs/SITE-DATA.md) · [Provas, incluindo falhas corrigidas](docs/evidence/site-data). Contribuições multiutilizador continuam por implementar; a biblioteca de recursos está em validação nesta branch.
