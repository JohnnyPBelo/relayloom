...
Nesta branch, a criação local de **recursos opcionais** já está ligada aos motores Node, Go e browser: preparação cifrada, recuperação do mesmo ID e transferência apenas mediante pedido. Passaram 450 testes Node, Go sites/app com race, 19 de interoperabilidade, 57 casos de browser e 23 percursos UI de regressão; a biblioteca no editor, o documento v3 e a leitura por referência assinada ainda faltam. Estes recursos **não estão na interface pública**. [Contrato e limites](docs/SITE-RESOURCES.md) · [Provas e comandos](docs/evidence/site-optional-resources/creation).

O código actual inclui um novo bloco **Tabela**: colunas tipadas, edição de linhas, CSV/JSON, pesquisa, ordenação e paginação, dentro de versões assinadas. A extensão passou 138 testes Node, Go/race, 11 de interoperabilidade, oito percursos UI Node/Go e 108 de browser. **Já está na versão web publicada** (fonte 59c9bd1, distribuição f6758222). [Guia e limites](docs/SITE-DATA.md) · [Provas, incluindo falhas corrigidas](docs/evidence/site-data). Contribuições multiutilizador e a interface de ficheiros opcionais continuam por implementar.
