...
A próxima extensão são **formulários e contribuições assinadas**. O código já liga formulários a tabelas autenticadas e guarda intenções de propostas privadamente em Node/browser antes de assinar. A consulta do formulário funciona nos três motores. **Ainda faltam o catálogo Go completo, envio, caixa de propostas e aprovação na interface; o HTML público não oferece esse fluxo.** O marco local passou 494 testes Node e os controlos afectados nos browsers. [Estado funcional e limites](docs/SITE-CONTRIBUTIONS.md) · [Provas e falhas corrigidas](docs/evidence/site-contributions/preparation).
