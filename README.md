...
No checkpoint `274004e`, o CI34721962376 passou Node/Go e os três pacotes desktop. No simulador iOS26.4.1, a fotografia foi importada e o XCUITest lançou a aplicação; o teste falhou ao esperar pela WKWebView. Nenhum fluxo funcional iOS passou; ver [STATUS](docs/STATUS.md).

A gestão de membros por épocas tem bibliotecas Node/Go e uma API local autenticada, com convite, consentimento, saída/remoção/reentrada e persistência transaccional cifrada. Os testes usam processos e ficheiros reais, incluindo troca de núcleo no reinício. A recepção/admissão por épocas já usa transacções e reservas físicas verificadas em Node/Go. A interface ainda usa grupos fixos; envio/outbox dinâmica, sincronização automática das provas e UI continuam pendentes. [GROUP-RUNTIME](docs/GROUP-RUNTIME.md) separa estas capacidades e os testes executados.
...
- As conversas da interface têm membros fixos. A [gestão por épocas](docs/GROUP-RUNTIME.md) está implementada na API, mas ainda não entrega mensagens dinâmicas pela rede/UI. Rotação/revogação também não está concluída. Máximo 64 destinatários, 4 MiB por objecto, quatro anexos por mensagem e 2 MB por anexo na interface.
