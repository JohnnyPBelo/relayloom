...
A próxima extensão são **formulários e contribuições assinadas**. Os motores Node/Go/browser já têm fila e inbox privadas recuperáveis, obtenção da origem na fila do visitante e descarte local com protecção contra reenvios, sem transferir autoria. A última regressão incluiu 46 testes Node, Go/race, 87 testes entre processos e, após corrigir uma corrida de autorização, 89 casos por navegador. **Faltam recibos, aprovação/recusa comunicada, reconciliação e o fluxo completo na interface; o HTML público ainda não oferece esse fluxo.** [Estado funcional](docs/SITE-CONTRIBUTIONS.md) · [Provas e limites](docs/evidence/site-contributions/dismissal).
