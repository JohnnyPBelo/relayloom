...
A próxima extensão são **formulários e contribuições assinadas**. Os motores Node/Go/browser já têm fila e inbox privadas, obtenção da origem no visitante, descarte protegido contra reenvios e preparação recuperável de recibos assinados. A persistência passou 526 testes Node, Go/race, 117 testes entre processos e 97 casos por navegador. **Faltam entrega automática dos recibos, aprovação/recusa comunicada, reconciliação e o fluxo completo na interface; o HTML público ainda não oferece esse fluxo.** [Estado funcional](docs/SITE-CONTRIBUTIONS.md) · [Provas e limites](docs/evidence/site-contributions/receipt-persistence).
