...
A próxima extensão são **formulários e contribuições assinadas**. Já há fila de envio e inbox privadas recuperáveis em Node/Go/browser, com assinatura original, conflitos e fonte autenticada preservados após perder a cache. A origem também pode ser pedida à fila privada do visitante, com relay de terceiros pausado. A integração passou 514 testes Node, Go/race, 66 testes entre processos e 60 percursos por browser; uma correcção de admissão teve gate posterior com proveniência separada. **Faltam recibos, aprovação/rejeição, reconciliação e o fluxo completo na interface; o HTML público não oferece esse fluxo.** [Estado funcional](docs/SITE-CONTRIBUTIONS.md) · [Provas e limites](docs/evidence/site-contributions/inbox).
